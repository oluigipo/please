	...
	GLsizei length, const GLchar* message, const void* userParam)
{
	if (type == GL_DEBUG_TYPE_ERROR)
		OS_DebugMessageBox("OpenGL Error:\nType = 0x%x\nID = %u\nSeverity = 0x%x\nMessage= %s",
		type, id, severity, message);
	else
		OS_DebugLog("OpenGL Debug Callback:\n\tType = 0x%x\n\tID = %u\n\tSeverity = 0x%x\n\tmessage = %s\n",
		type, id, severity, message);
}
#endif
...
OpenGL_MakeShader(const Render_ShaderDesc* desc, Render_Shader* out_shader)
{
	...
	if (!success) {
		...
		OS_DebugMessageBox("Vertex Shader Error:\n\n%s", info);
	...
	if (!success) {
		...
		OS_DebugMessageBox("Fragment Shader Error:\n\n%s", info);
	...
	if (!success) {
		...
		OS_DebugMessageBox("Shader Linking Error:\n\n%s", info);
