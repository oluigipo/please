...
typedef GltfJson_Root;

//~ Functions
static inline const uint8*
Gltf_ReadUint32_(const uint8* begin, const uint8* end, uint32* out_value)
{
...
static void
Gltf_ParseDataAsset_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
...
static bool
Gltf_ParseDataNode_(Arena* arena, const Json_Value* value, GltfJson_Node* out)
{
...
static bool
Gltf_ParseDataNodes_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	...
	out->nodes = Arena_Push(arena, sizeof(GltfJson_Node) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataNode_(arena, &index_value, &out->nodes[i]))
			return false;
	}
...
static bool
Gltf_ParseDataScene_(Arena* arena, const Json_Value* value, GltfJson_Scene* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataScenes_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	if (value->kind != Json_ValueKind_Array)
		return false;
	...
	out->scenes = Arena_Push(arena, sizeof(GltfJson_Scene) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataScene_(arena, &index_value, &out->scenes[i]))
			return false;
	}
...
static bool
Gltf_ParseDataTextureInfo_(Arena* arena, const Json_Value* value, GltfJson_TextureInfo* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataMetallicRoughness_(Arena* arena, const Json_Value* value, GltfJson_MetallicRoughness* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
	...
	for (Json_Field field = { value }; Json_NextField(&field); )
	{
		...
		{
			if (!Gltf_ParseDataTextureInfo_(arena, &field_value, &out->base_color_texture))
				return false;
		}
		else if (String_Equals(field_name, Str("metallicRoughnessTexture")))
		{
			if (!Gltf_ParseDataTextureInfo_(arena, &field_value, &out->metallic_roughness_texture))
				return false;
		}
	}
...
static bool
Gltf_ParseDataNormalTextureInfo_(Arena* arena, const Json_Value* value, GltfJson_NormalTextureInfo* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataOcclusionTextureInfo_(Arena* arena, const Json_Value* value, GltfJson_OcclusionTextureInfo* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataMaterial_(Arena* arena, const Json_Value* value, GltfJson_Material* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
	...
	for (Json_Field field = { value }; Json_NextField(&field); )
	{
		...
		{
			if (!Gltf_ParseDataMetallicRoughness_(arena, &field_value, &out->pbr_metallic_roughness))
				return false;
		}
		else if (String_Equals(field_name, Str("normalTexture")))
		{
			if (!Gltf_ParseDataNormalTextureInfo_(arena, &field_value, &out->normal_texture))
				return false;
		}
		else if (String_Equals(field_name, Str("occlusionTexture")))
		{
			if (!Gltf_ParseDataOcclusionTextureInfo_(arena, &field_value, &out->occlusion_texture))
				return false;
		}
		else if (String_Equals(field_name, Str("emissiveTexture")))
		{
			if (!Gltf_ParseDataTextureInfo_(arena, &field_value, &out->emissive_texture))
				return false;
		}
...
static bool
Gltf_ParseDataMaterials_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	if (value->kind != Json_ValueKind_Array)
		return false;
	...
	out->materials = Arena_Push(arena, sizeof(GltfJson_Material) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataMaterial_(arena, &index_value, &out->materials[i]))
			return false;
	}
...
static bool
Gltf_ParseDataPrimitive_(Arena* arena, const Json_Value* value, GltfJson_Primitive* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataMesh_(Arena* arena, const Json_Value* value, GltfJson_Mesh* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
	...
	for (Json_Field field = { value }; Json_NextField(&field); )
	{
		...
		else if (String_Equals(field_name, Str("primitives")))
		{
			...
			uintsize length = Json_ArrayLength(&field_value);
			out->primitives = Arena_Push(arena, sizeof(GltfJson_Primitive) * length);
			...
			for (Json_ArrayIndex index = { &field_value }; Json_NextIndex(&index); ++i)
			{
				...
				if (!Gltf_ParseDataPrimitive_(arena, &index_value, &out->primitives[i]))
					return false;
			}
		}
	}
...
static bool
Gltf_ParseDataMeshes_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	if (value->kind != Json_ValueKind_Array)
		return false;
	...
	out->meshes = Arena_Push(arena, sizeof(GltfJson_Mesh) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataMesh_(arena, &index_value, &out->meshes[i]))
			return false;
	}
...
static bool
Gltf_ParseDataAccessor_(Arena* arena, const Json_Value* value, GltfJson_Accessor* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataAccessors_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	if (value->kind != Json_ValueKind_Array)
		return false;
	...
	out->accessors = Arena_Push(arena, sizeof(GltfJson_Accessor) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataAccessor_(arena, &index_value, &out->accessors[i]))
			return false;
	}
...
static bool
Gltf_ParseDataSampler_(Arena* arena, const Json_Value* value, GltfJson_Sampler* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataSamplers_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	if (value->kind != Json_ValueKind_Array)
		return false;
	...
	out->samplers = Arena_Push(arena, sizeof(GltfJson_Sampler) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataSampler_(arena, &index_value, &out->samplers[i]))
			return false;
	}
...
static bool
Gltf_ParseDataImage_(Arena* arena, const Json_Value* value, GltfJson_Image* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataImages_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	if (value->kind != Json_ValueKind_Array)
		return false;
	...
	out->images = Arena_Push(arena, sizeof(GltfJson_Image) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataImage_(arena, &index_value, &out->images[i]))
			return false;
	}
...
static bool
Gltf_ParseDataTexture_(Arena* arena, const Json_Value* value, GltfJson_Texture* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataTextures_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	if (value->kind != Json_ValueKind_Array)
		return false;
	...
	out->textures = Arena_Push(arena, sizeof(GltfJson_Texture) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataTexture_(arena, &index_value, &out->textures[i]))
			return false;
	}
...
static bool
Gltf_ParseDataBufferView_(Arena* arena, const Json_Value* value, GltfJson_BufferView* out)
{
	if (value->kind != Json_ValueKind_Object)
		return false;
...
static bool
Gltf_ParseDataBufferViews_(Arena* arena, const Json_Value* value, GltfJson_Root* out)
{
	if (value->kind != Json_ValueKind_Array)
		return false;
	...
	out->buffer_views = Arena_Push(arena, sizeof(GltfJson_BufferView) * length);
	...
	for (Json_ArrayIndex index = { value }; Json_NextIndex(&index); ++i)
	{
		...
		if (!Gltf_ParseDataBufferView_(arena, &index_value, &out->buffer_views[i]))
			return false;
	}
...
static bool
Gltf_ParseData_(Arena* arena, const uint8* json_begin, const uint8* json_end,
	const uint8* binary_begin, const uint8* binary_end, GltfJson_Root* out)
{
	Json_Value json;
	...
	for (Json_Field field = { &json }; Json_NextField(&field); )
	{
		...
		{
			Gltf_ParseDataAsset_(arena, &field_value, out);
		}
		...
		{
			if (!Gltf_ParseDataScenes_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("nodes")))
		{
			if (!Gltf_ParseDataNodes_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("materials")))
		{
			if (!Gltf_ParseDataMaterials_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("meshes")))
		{
			if (!Gltf_ParseDataMeshes_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("accessors")))
		{
			if (!Gltf_ParseDataAccessors_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("samplers")))
		{
			if (!Gltf_ParseDataSamplers_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("images")))
		{
			if (!Gltf_ParseDataImages_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("textures")))
		{
			if (!Gltf_ParseDataTextures_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("bufferViews")))
		{
			if (!Gltf_ParseDataBufferViews_(arena, &field_value, out))
				return false;
		}
		else if (String_Equals(field_name, Str("buffers")))
		{
			...
			out->buffers = Arena_Push(arena, sizeof(GltfJson_Buffer) * length);
...
//~ API
static bool
Gltf_Parse(const uint8* data, uintsize size, Arena* arena, GltfJson_Root* out)
{
	Trace();
	...
	begin = Gltf_ReadUint32_(begin, end, &magic);
	begin = Gltf_ReadUint32_(begin, end, &version);
	begin = Gltf_ReadUint32_(begin, end, &length);
	...
	while (begin + 8 < end)
	{
		...
		begin = Gltf_ReadUint32_(begin, end, &chunk_length);
		begin = Gltf_ReadUint32_(begin, end, &chunk_type);
	...
	bool result = Gltf_ParseData_(arena, json_begin, json_end, binary_begin, binary_end, out);
